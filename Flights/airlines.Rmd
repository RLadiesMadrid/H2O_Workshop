...
h2o.confusionMatrix(flights.glm, valid=TRUE)
```
```{r roc_curve}
...
fpr_val <- flights.regularized@model$validation_metrics@metrics$thresholds_and_metric_scores$fpr
tpr_val <- flights.regularized@model$validation_metrics@metrics$thresholds_and_metric_scores$tpr
...
flights_glm_r <- h2o.glm(model_id = "regularized", training_frame = data$Train, 
                         validation_frame = data$Valid, x = x, y = y,
                         family='binomial', lambda_search = TRUE, solver = 'COORDINATE_DESCENT')
```
